...
`;
        ...
        const filePath = path.join(this.config.outputPath, `${this.toSnakeCase(this.config.className)}.dart`);
...
`;
        ...
        const filePath = path.join(this.config.outputPath, `${this.toSnakeCase(module.name)}_l10n.dart`);
        fs.writeFileSync(filePath, content, 'utf-8');
    }
        ...
        for (const locale of this.config.supportedLocales) {
            ...
            for (const module of modules) {
                for (const key of module.keys) {
                    const translation = key.translations[locale] || key.translations[this.config.defaultLocale] || '';
        ...
        const messageEntries = Object.entries(messages)
            .map(([key, value]) => {
                ...
                if (isIcu) {
                    ...
                    const processedValue = this.escapeDartString(value)
                        .replace(/\{(\w+)\}/g, '$$$1');
        ...
        for (const locale of this.config.supportedLocales) {
            ...
            for (const module of modules) {
                for (const key of module.keys) {
                    ...
                    combinedArb[fullKey] = key.translations[locale] || key.translations[this.config.defaultLocale] || '';
